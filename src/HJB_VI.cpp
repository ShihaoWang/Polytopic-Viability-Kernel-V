...
double LdotLow = -2.0;          double LdotUpp = 2.0;
double ThetaLow = -PI/2.0;      double ThetaUpp = -1.0 * ThetaLow;
double ThetadotLow = -2.0;      double ThetadotUpp = -1.0 * ThetadotLow;

const int L_Grids = 60;
const int Ldot_Grids = 60;
const int Theta_Grids = 60;
const int Thetadot_Grids = 60;
...
const int F_Grids = 31;       // This is the discretization of the control points within a range
double delta_t = 0.1;
...
void FailureMetricUpdate(DBNode& Node_k)
{
  ...
  if((Theta_k>0)&&(Thetadot_k>=0))
  {
    // This is the preferred cost.
    // Node_k.FailureMetric = -1.0 * L_k * cos(Theta_k);
        ...
        for (int j = 0; j < Ldot_Grids; j++)
        {
          for (int k = 0; k < Theta_Grids; k++)
          {
            for (int l = 0; l < Thetadot_Grids; l++)
            {
              ...
              // double FailureMetricVia_i = Forward_Evaluation(StateNodeMatrix(29,35,34,30), StateNodeMatrix);
              // double FailureMetricVia_i = Forward_Evaluation(StateNodeMatrix(26,28,27,25), StateNodeMatrix);
